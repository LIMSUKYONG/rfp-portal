...
CREATE POLICY "projects_member_access" ON rfp_projects
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM rfp_project_members pm
      JOIN rfp_users u ON u.id = pm.user_id
      WHERE pm.project_id = rfp_projects.id AND u.auth_uid = auth.uid()
    )
    OR EXISTS (
      SELECT 1 FROM rfp_users u
      WHERE u.auth_uid = auth.uid() AND u.role = 'pm'
...
CREATE POLICY "projects_pm_modify" ON rfp_projects
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM rfp_users u
      WHERE u.auth_uid = auth.uid() AND u.role = 'pm'
