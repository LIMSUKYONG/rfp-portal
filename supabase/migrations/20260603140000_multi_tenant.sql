...
-- 5. RLS 정책 — tenant isolation (모든 테이블)
-- Helper function: 현재 사용자의 tenant_id 추출
CREATE OR REPLACE FUNCTION current_tenant_id() RETURNS UUID AS $$
  SELECT (auth.jwt() ->> 'tenant_id')::uuid;
$$ LANGUAGE sql STABLE SECURITY DEFINER;

-- 각 테이블에 tenant_isolation 정책 적용
DO $$
DECLARE
  tbl TEXT;
BEGIN
...
-- 6. JWT custom claim — 로그인 시 tenant_id 자동 포함
