...
test.describe("SCR-502 협상 관리 화면", () => {
  ...
  test("제목이 표시된다", async ({ page }) => {
    ...
    if (t) expect(["협상 관리", "404"]).toContain(t.trim());
  });
});

test.describe("SCR-502 컴포넌트 소스 검증", () => {
  test("data-testid가 모두 정의되어 있다", async () => {
    const p = fs.readFileSync(path.resolve(__dirname, "../../src/app/projects/[id]/negotiation/page.tsx"), "utf-8");
    const c = fs.readFileSync(path.resolve(__dirname, "../../src/app/projects/[id]/negotiation/_components/negotiation-manager.tsx"), "utf-8");
    const s = p + c;
  ...
  test("협상 범위 상한/하한이 표시된다", async () => {
    const s = fs.readFileSync(path.resolve(__dirname, "../../src/app/projects/[id]/negotiation/_components/negotiation-manager.tsx"), "utf-8");
