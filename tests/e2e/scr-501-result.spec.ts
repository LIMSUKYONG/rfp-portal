...
test.describe("SCR-501 입찰 결과 등록 화면", () => {
  ...
  test("제목이 표시된다", async ({ page }) => {
    ...
    if (t) expect(["입찰 결과 등록", "404"]).toContain(t.trim());
  });
});

test.describe("SCR-501 컴포넌트 소스 검증", () => {
  test("data-testid가 모두 정의되어 있다", async () => {
    const p = fs.readFileSync(path.resolve(__dirname, "../../src/app/projects/[id]/result/page.tsx"), "utf-8");
    const c = fs.readFileSync(path.resolve(__dirname, "../../src/app/projects/[id]/result/_components/result-form.tsx"), "utf-8");
    const s = p + c;
  ...
  test("미선정 시 탈락 사유 입력이 필수이다", async () => {
    const s = fs.readFileSync(path.resolve(__dirname, "../../src/app/projects/[id]/result/_components/result-form.tsx"), "utf-8");
